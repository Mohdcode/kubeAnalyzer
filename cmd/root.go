...
var RootCmd = &cobra.Command{
	...
	Long: `kubeAnalyzer is a powerful Kubernetes resource health checker that helps developers and operators maintain the health and stability of their Kubernetes environments. It analyzes Kubernetes resources, including Deployments, StatefulSets, ConfigMaps, Pods, and Services, to identify deprecated or removed APIs, potential issues with Pods, and misconfigured Services.

	With kubeAnalyzer, you can proactively ensure your Kubernetes resources are up-to-date with the latest APIs and best practices, reducing the risk of unexpected failures and enhancing overall cluster reliability. The tool offers user-friendly reports with actionable insights, making it easy to identify and address potential problems.

	Example:
	$ kubeAnalyzer check apis
	$ kubeAnalyzer check pods
	$ kubeAnalyzer check services

	Start using kubeAnalyzer today to keep your Kubernetes resources healthy and optimize your cluster's performance.
	`,
...
func init() {
	...
	// rootCmd.PersistentFlags().StringVar(&cfgFile, "config", "", "config file (default is $HOME/.kubeAnalyzer.yaml)")
